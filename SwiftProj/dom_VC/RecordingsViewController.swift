        ...
        actionsheet.addAction(UIAlertAction(title: "Transcribe to Note", style: UIAlertAction.Style.default, handler: { (action) -> Void in
            ...
            if (recognizer?.isAvailable)! {
                ...
                recognizer?.recognitionTask(with: request) { result, error in
                    ...
                    self.fsdbManager.addNote(titleStr: recording.name, bodyStr: result.bestTranscription.formattedString , tagStr: "", uid: self.user?.uid, noteUpdateDate: dateText, noteImageURL: "")
